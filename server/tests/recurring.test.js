...
const createTpl = async (overrides = {}) => {
  const res = await api.post('/api/recurring').set(auth).send({
...
describe('PATCH /api/recurring/:id', () => {
  test('暂停模板返回更新后的数据并持久化', async () => {
    ...
    const list = await api.get('/api/recurring').set(auth);
    expect(list.body.data.find((t) => t.id === tpl.id).is_active).toBe(0);
  });

  test('恢复已暂停的模板', async () => {
    const tpl = await createTpl();
    await api.patch(`/api/recurring/${tpl.id}`).set(auth).send({ is_active: 0 });
    const res = await api
  ...
  test('未登录返回 401', async () => {
    const res = await api.patch('/api/recurring/1').send({ is_active: 0 });
...
describe('POST /api/recurring/generate 与暂停状态', () => {
  test('活跃且到期的模板会生成，同日不重复生成', async () => {
    ...
    const first = await api.post('/api/recurring/generate').set(auth);
    expect(first.body.generated).toBe(1);
    const second = await api.post('/api/recurring/generate').set(auth);
    expect(second.body.generated).toBe(0);
  });

  test('已暂停的到期模板不会生成', async () => {
    insertDueToday({ is_active: 0 });
    const res = await api.post('/api/recurring/generate').set(auth);
  ...
  test('恢复后即可生成', async () => {
    ...
    const before = await api.post('/api/recurring/generate').set(auth);
    expect(before.body.generated).toBe(0);

    await api.patch(`/api/recurring/${id}`).set(auth).send({ is_active: 1 });
    const after = await api.post('/api/recurring/generate').set(auth);
