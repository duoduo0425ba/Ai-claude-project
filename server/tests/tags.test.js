...
beforeAll(async () => {
  const resA = await api.post('/api/auth/register').send({
    username: 'taguser_a', password: 'password123',
  });
  authA = { Authorization: `Bearer ${resA.body.data.token}` };

  const resB = await api.post('/api/auth/register').send({
...
const add = (t, auth = authA) =>
  api.post('/api/transactions').set(auth).send(t);

const getTags = (auth = authA) => api.get('/api/tags').set(auth);
...
describe('POST /api/transactions 带标签', () => {
  it('创建时写入标签并按名称排序返回', async () => {
    ...
    const list = await api.get('/api/transactions').set(authA);
...
describe('GET /api/transactions 附带标签', () => {
  it('每行只带自己的标签', async () => {
    ...
    const res = await api.get('/api/transactions').set(authA);
...
describe('跨用户隔离', () => {
  ...
  it('用户 B 筛选不到用户 A 的标签，列表行也不带 A 的标签', async () => {
    ...
    const list = await api.get('/api/transactions').set(authB);
...
describe('POST /api/transactions/batch 带标签', () => {
  ...
  it('备份导出的原始行（含 id/created_at/user_id/tags）可回灌且标签保留', async () => {
    ...
    const exported = await api.get('/api/transactions').set(authA);
...
describe('管理员删除用户时级联清理标签', () => {
  it('删除用户后其 tags 与 transaction_tags 无残留', async () => {
    ...
    const resC = await api.post('/api/auth/register').send({
